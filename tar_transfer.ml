...
and transform_files ~from_tar ~src_manifest ~dst_dir ~user ~to_untar =
  ...
  let fname file_name =
    ...
    | file_name -> file_name
  in
  Tar_unix.Archive.transform ~level (transform ~user fname) from_tar to_untar
...
let transform_file ~from_tar ~src_manifest ~dst ~user ~to_untar =
  ...
  Tar_unix.Archive.transform ~level (fun hdr ->
      ...
      Logs.debug (fun f -> f "Copying %s -> %s@." hdr.Tar.Header.file_name hdr'.Tar.Header.file_name);
      hdr')
    from_tar to_untar
