...
let rec check_path ~acc base = function
  ...
  | x :: _ when String.contains x platform_dir_sep ->
      Fmt.error_msg
        "Can't use platform directory separator in path component: %S" x
  | x :: xs -> (
      let path = base / x in
      let acc = x :: acc in
      match Unix.lstat path with
      ...
      | Unix.{ st_kind = S_REG; _ } ->
          Fmt.error_msg "Not a directory: %a" pp_rev_path acc
...
let generate ~exclude ~src_dir src =
  ...
  | Ok src' -> (
      try
        ...
        |> generate ~exclude ~src_dir |> Result.ok
